    ...
    <van-popup v-model="dialogShow">
      ...
      >
        <div class="van-dialog__content">
          <div class="van-dialog__message">是否保存{{sessionName}}服务团队名片简介?</div>
        </div>
        <div
  ...
  export default {
    ...
    watch: {
      message(value) {
        ...
        if(value.length > 100) {
          Toast(`请编辑${this.sessionName}服务医疗团队名片简介0-100字`);
    ...
    methods: {
      ...
      updateDoctorGroupDetail() {
        this.$http.request({
          ...
          body: {
            id: this.groupId,
            sessionintroduction:this.message,
    ...
    created() {
      ...
      if(this.$route.query.docGroupCardMessage) {
        this.message = this.$route.query.docGroupCardMessage;
      }
