...
export default {
  ...
  methods: {
    ...
    getQuestionnaireData() {
      ...
      this.$http
        .request({
          method: "get",
          url: "/TaskQueryController/selectTaskQueryAnswer",
          body: {
            id: this.id
          }
        })
        .then(res => {
          if (res.data.success && res.data.data) {
            this.loading = false;
            let copyData = JSON.parse(JSON.stringify(res.data.data.data));
            copyData &&
            ...
            this.questionnaireData = copyData;
            this.pageTitle =
              res.data.data.data[0] && res.data.data.data[0].title;
          } else {
            console.log("调查问卷答案查询接口报错");
          }
        })
        .catch(e => {
          this.loading = false;
  ...
  created() {
    if (this.$route.query.id) {
      ...
      localStorage.setItem("questionnaireDetailId", this.id);
      this.getQuestionnaireData();
    } else {
      this.id = localStorage.getItem("questionnaireDetailId");
      this.getQuestionnaireData();
    }
  }
};
