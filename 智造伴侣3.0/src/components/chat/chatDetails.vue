<template>
  <div class="chatDetails_page">
    ...
    <div class="im_container">
      <div class="chat-item clearFix" v-for="(item, index) in chatList" :key="index">
        ...
        <div
          ...
          :class="item.userId !== loginData.userMessage.userid ? 'itemLeft' : 'itemRight'"
        >
        ...
        <div class="sys-msg-box" v-else @click.stop="systemMsg(JSON.parse(item.content))">
          <img src="../../assets/image/icon-sys-msg.png" alt="">
          <span>{{JSON.parse(item.content).text}}</span>
        </div>
      </div>
    ...
    <!-- 保存文件到网盘 弹窗 -->
    <send-file-dialog
      :isFileSaveCloud="true"
      v-if="moveFolderDialogShow"
      :filterFileSapceList="filterFileSpaceList"
...
<script>
import shareFileDialog from "../CloudDisk/shareFileDialog";
import sendFileDialog from "./sendFileDialog";
import audioStaticImg from "../../assets/image/icon_audio_static.png";
import audioPlayImg from "../../assets/image/icon_audio_playing.gif";
...
export default {
  ...
  computed: {
    ...
    /**
     * @Description: 过滤文件空间列表(是否有保存权限)
     * @Author: 侯湃
     * @Date: 2020/6/8
     */
    filterFileSpaceList() {
      let filterFileSapceList = this.homeData.filterFileSpaceList || [];
      filterFileSapceList = filterFileSapceList.filter(v => {
        return v.permission.includes("4");
      });
      return filterFileSapceList;
    }
  },
  props: {
    chatList: {
  ...
  methods: {
    ...
    submitShare(val) {
      ...
      if (this.selectItem.messageType !== 4) {
        ...
        let par = {
          fileId: JSON.parse(this.selectItem.content).id,
          userList: userList,
          groupList: groupList,
    ...
    saveLocal() {
      ...
      cloudDiskSrv
        .submitDownloadFile(par)
        .then(res => {
          if (res.success) {
            location.href =
              this.$globalServiceHost.fileDiskServiceHost + "/file" + res.data;
            window.addEventListener("beforeunload", vm.$returnValue, false);
          } else {
            this.$message.error(res.message || "文件下载接口报错");
          }
        })
        .catch(err => {
          console.log(err);
    ...
    // 上传到网盘,如果没有可保存的文件空间,给出提示
    saveCloud() {
      if (!this.filterFileSpaceList.length) {
    ...
    // 上传网盘提交
    submitSaveCloudFile(file) {
      console.log("file: ", file);
      if (file.parentid === JSON.parse(this.selectItem.content).id) {
      ...
      let par = {
        ...
        fileId: JSON.parse(this.selectItem.content).id,
        token: localStorage.getItem("serverToken"),
        targetFileSpaceId: file.fileSpaceId
      };
      homeSrv
        .saveToMyWP(par)
        .then(res => {
    ...
    // 点击系统消息
    systemMsg(content) {
      this.$bus.emit("clickSystemMessage", content);
    }
  }
};
</script>
