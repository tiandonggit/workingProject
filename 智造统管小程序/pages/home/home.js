...
Page({
  ...
  data: {
    ...
    someAreas: {}, //部分地区选择结果
    navH: "", //导航栏高度
    userType: 2, //  0：产品专员 1：运营商 2 医助 3：总管理员
    timeSelectText: "本月数据",
  ...
  onChange(event) {
    ...
    if (this.data.userType == 3) {
      ...
      this.setData({
        ...
        shaicha: shaicha
        // y_guanlian_patient_number: staticsData.patientNum || 0,
        // y_guanlian_doctor_number: staticsData.doctorNum || 0,
        // y_gui_number: staticsData.deviceNum || 0,
        // xiaji_yizhu_number: staticsData.assistantNum || 0,
        // guanlian_doctor_number: staticsData.doctorNum || 0,
        // guanlian_patient_number: staticsData.patientNum || 0,
        // xiaji_yunyingshang_number: staticsData.bussinessNum || 0,
        // yz_tuiguang_doctor_number:staticsData.tuiguangyishengshu || 0,
        // yz_guanlian_patient_number:staticsData.guanlianhuanzheshu || 0
      });
    }
  },
  ...
  goLocation() {
    wx.navigateTo({
      url: "../positionCheckIn/positionCheckIn"
    });
  },
  ...
  panelLink(e) {
    ...
    if (this.data.userType == 3) {
      ...
      if (this.data.userType == 1 && e.currentTarget.dataset.usertype) {
        ...
        wx.navigateTo({
          url: `../levelList/levelList?type=${type}`
        });
      }
    }
  },
  ...
  getYunYingShangData() {
    fly.request({
      ...
      if (res.data.success && res.data.data) {
        ...
        wx.setStorageSync('userAddress', userInfo.provName + '-' + userInfo.cityName);
        let shaicha = {};
        let yingyangtuijian = {};
        let tuijianchanpin = {};
        if (Object.keys(staticsData).length) {
  ...
  getYiZhuAddress() {
    fly.request({
      method: "get",
      url: serviceHost + "/RAssistantController/selectbyAssid",
      body: {
        id: wx.getStorageSync("userid")
      }
    }).then(res => {
      if(res.data.success && res.data.data) {
        ...
        wx.setStorageSync('userAddress', userInfo.provname + '-' + userInfo.cityaname);
      }
    }).catch(e => {
      console.log(e)
    })
    ...
    fly.request({
      ...
      if (res.data.success && res.data.data) {
        ...
        this.setData({
          ...
          yingyangtuijian: yingyangtuijian,
          shaicha: shaicha,
          guanlian_doctor_number: res.data.data.doctorNum,
          guanlian_patient_number: res.data.data.patientNum ,
