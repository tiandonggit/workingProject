...
export default {
  install(Vue) {
    ...
    _Vue.prototype.$timestampToTime = (timestamp) => {
      ...
      formatTime = y + '-' + m + '-' + d + ' ' + h + ':' + minute + ':' + second;
      let timeS = formatTime;
      let todayT = ''; //
      let yestodayT = '';
      let tYear = y + '-' + '01' + '-' + '01' + ' ' + '00' + ':' + '00' + ':' + '00';
      let tYearTimeStamp = new Date(tYear.replace(/[年月]/g, '-').replace(/[日]/, '').replace(/-/g, "/")).getTime();
      let timeSS = timeS.replace(/[年月]/g, '-').replace(/[日]/, '').replace(/-/g, "/");
